-- =============================================================================
-- fronius-bridge: per-device inverter schema (migration 003)
--
-- Two continuous aggregates of AC active power (total across phases). Each
-- carries the per-bucket average plus the minimum and maximum, so the dashboard
-- can draw a min/max envelope around the average power line:
--
...
SELECT time_bucket('5 minutes', time) AS bucket,
       avg(ac_power_active)           AS avg_power,  -- W, total AC active power
       min(ac_power_active)           AS min_power,
       max(ac_power_active)           AS max_power
FROM samples
GROUP BY bucket
WITH NO DATA;
