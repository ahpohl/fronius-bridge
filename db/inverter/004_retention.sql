-- =============================================================================
-- fronius-bridge: per-device inverter schema (migration 004)
--
...
--
--   samples / phase_samples / input_samples   compress 7d, drop 90 days
...
-- one row per day, never dropped, and daily.first_kwh/last_kwh preserve the
-- boundary counter readings the gap-estimator needs. Both aggregates are kept
-- 2 years: power_5min backs long-range plots, and power_30sec is the
-- high-fidelity source the simulated self-consumption split is summed from, so
-- it must outlive the raw samples to keep that recompute available.
--
...
-- especially well (delta-of-delta). segmentby groups the per-phase / per-input
-- series of the multi-series tables; the single-series samples and the
-- aggregates take none.
--
...
ALTER TABLE phase_samples SET (
    ...
    timescaledb.orderby            = 'time DESC');
ALTER TABLE input_samples SET (
    timescaledb.enable_columnstore = TRUE,
    timescaledb.segmentby          = 'input_id',
    timescaledb.orderby            = 'time DESC');
...
CALL add_columnstore_policy('phase_samples',
    after => INTERVAL '7 days', if_not_exists => TRUE);
CALL add_columnstore_policy('input_samples',
    after => INTERVAL '7 days', if_not_exists => TRUE);
...
SELECT add_retention_policy('phase_samples',
    drop_after => INTERVAL '90 days', if_not_exists => TRUE);
SELECT add_retention_policy('input_samples',
    drop_after => INTERVAL '90 days', if_not_exists => TRUE);
